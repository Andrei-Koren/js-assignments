...
function getDistanceBetweenPoints(x1, y1, x2, y2) {
    ...
    let fy1 = y1;
    let fy2 = y2;
    let value = Math.sqrt(Math.pow((fx2 - fx1), 2) + Math.pow((fy2 - fy1), 2));
    return value;   
}
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
	let a1 = x1*x2 + y1 * y2;
	let a2 = Math.sqrt(Math.pow(x1, 2) + Math.pow(y1, 2));
	let a3 = Math.sqrt(Math.pow(x2, 2) + Math.pow(y2, 2));
	let cosAngle = a1 / (a2 * a3);
...
function getParallelipidedDiagonal(a,b,c) {
   let diog = Math.sqrt(Math.pow(a, 2) + Math.pow(b, 2) + Math.pow(a, 2));
   return diog;
}
